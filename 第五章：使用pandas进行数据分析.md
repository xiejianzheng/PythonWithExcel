...
除了mean，你还可以使用pandas提供的大多数描述性统计信息，如果你想
使用自己的函数，可以使用agg方法。例如，以下是如何获得每个分组
最大值和最小值差值：

df.groupby(["大陆"]).agg(lambda x: x.max() - x.min())

在Excel中获取每组统计值的常用方法是透视表。他们引入了第二个维度，
是从不同角度看数据很好的方式。pandas也有个数据透视表功能，我们将
在下面看到。

### 旋转和熔化

如果你在Excel中使用透视表，你使用pan的透视表函数不会有问题，因为它们的工作方式基本相同。在以下数据帧中的数据的组织方式与数据库中记录的存储方式类似；每行显示一条特定地区特定水果的销售记录。

要创建透视表，你需要将数据帧作为pivot_table的第一个参数。索引和列
参数分别定义了数据帧的哪一列将分别成为透视表的行和列标签。值将通过aggfunc聚合到结果数据帧的数据部分，aggfunc是一个可以作为字符串或NumPy ufunc提供的函数。最后，margins（边距）参数Excel中的Grand Total总计相对应。如果你不填边距和边距名，则总计列和行将不会显示：

总之，数据透视意味着获取列的唯一值，并将其转换成透视表的列标签，
从而聚合来自另一列的值。这使得跨感兴趣的维度读取信息变得容易。
如果你想走另一条路，将列标题变成某一列的值，使用melt融合函数。
从这个意义上说，melt与pivot_table功能相反：
...
在这里，我把我们的透视表作为输入，但我使用iloc去除了汇总行和列。
我还重置了索引，以便所有的信息都可以作为普通列使用。我提供了
id_vars用来标识标识符，还提供了"值变量value_vars"用来定义我想
反透视的列。如果你希望准备数据，以便它可以存储回一个这种格式的
数据库中，则融合非常有用。

使用聚合统计，能帮助你理解你的数据，但没有人喜欢阅读整页的数字。
为了使信息易于理解，没有什么比创建可视化效果更好的了，这是我们
的下一个主题。虽然Excel使用术语图表，但pandas通常将其称为plots
绘图。我会在这本书中，互换使用这些术语。

### 绘图
绘图允许你可视化你的数据分析结果，可能是整个过程中最重要的步骤。
为了绘图，我们将使用两个库：我们先看Matplotlib，pandas的默认
绘图类库，然后关注Plotly，一个现代的绘图类库，给我们在Jupyter笔记
本中提供更具交互性的体验。


#### Matplotlib
Matplotlib是一个已经存在很久的绘图包，并且已经包含在了Anaconda
发行版本中。使用它，你可以生成很多种格式的绘图，包括用于高质量
打印的矢量图形。当你调用数据帧的plot方法时，pandas会默认生成一个
matplotlib绘图。要在Jupyter笔记本中使用Matplotlib，你需要首先
运行两个魔术命令之一：%Matplotlib inline或%matplotlib notebook。
它们配置笔记本，以便在笔记本中显示绘图。后面一个命令增加了一些
交互性，允许你更改图表的大小或缩放因子。让我们开始使用pandas和
Matplotlib创建第一个绘图：

注意，在这个示例中，我使用了一个NumPy阵列来构造一个数据帧，
提供NumPy阵列允许你用上上一章介绍的NumPy构造函数；在这里，
我们使用NumPy生成基于伪随机数的pandas数据帧。因此，当你在你的
终端上运行示例时，你将得到不同的值。
